...
const Map = forwardRef(({ darkMode, isMobile, t }, ref) => {
  ...
  useEffect(() => {
    ...
    if (!token) {
      setError('Token Mapbox mancante. Aggiungi NEXT_PUBLIC_MAPBOX_TOKEN.');
    } else {
  ...
  useEffect(() => {
    ...
    Object.entries(locations).forEach(([category, places]) => {
      places.forEach((place, idx) => {
        ...
        // Popup migliorato (leggibile)
        const popup = new mapboxgl.Popup({ offset: 25, closeButton: true, closeOnClick: false })
          .setHTML(`
            <div style="font-family:sans-serif;min-width:180px;max-width:220px;background:white;border-radius:12px;padding:8px 12px;box-shadow:0 2px 8px rgba(0,0,0,0.15);">
              <div style="font-weight:bold;font-size:1rem;margin-bottom:4px;color:#1E3A5F;">${place.name}</div>
              <div style="font-size:0.8rem;color:#555;margin-bottom:4px;">${place.address}</div>
              <div style="font-size:0.7rem;color:#888;display:inline-block;background:#f0f0f0;padding:2px 8px;border-radius:20px;">${categoryNames[category]}</div>
            </div>
          `);
  ...
  // Funzione per centrare la mappa da esterno
  const handleSearchSelect = (coords) => {
    if (map.current) {
      map.current.flyTo({ center: [coords[0], coords[1]], zoom: 16, duration: 1200 });
      setTimeout(() => {
        ...
        Object.values(markers.current).forEach(marker => {
          ...
          const dist = Math.hypot(lngLat.lng - coords[0], lngLat.lat - coords[1]);
          if (dist < minDist) {
            minDist = dist;
            closestMarker = marker;
          }
        });
        if (closestMarker) closestMarker.togglePopup();
      }, 1300);
    }
  };
  ...
  // Costruisci l'oggetto locations per la ricerca (uguale a quello usato per i marker)
  const locationsForSearch = {
    beachClub: [
      { name: "Kalua", coords: [25.349664, 37.408069] },
      { name: "SantAnna", coords: [25.348914, 37.408450] },
      { name: "Tropicana", coords: [25.356731, 37.410286] },
      { name: "Scorpios", coords: [25.347631, 37.407244] },
      { name: "Alemagou", coords: [25.380850, 37.464667] },
      { name: "Nammos", coords: [25.337111, 37.415597] },
      { name: "Principote", coords: [25.359892, 37.476622] },
      { name: "Super Paradise", coords: [25.369467, 37.415503] },
      { name: "Anios", coords: [25.344194, 37.414403] },
      { name: "Branco", coords: [25.345183, 37.414025] },
      { name: "Thalas", coords: [25.368308, 37.415119] }
    ],
    restaurant: [
      { name: "Lío", coords: [25.328236, 37.444994] },
      { name: "Interni", coords: [25.328708, 37.445589] },
      { name: "Mediterraneo", coords: [25.328706, 37.445592] },
      { name: "Zuma Mykonos", coords: [25.329533, 37.458539] },
      { name: "Cavo Tagoo", coords: [25.328942, 37.456669] },
      { name: "Spilia", coords: [25.419383, 37.434731] },
      { name: "Carosello", coords: [25.328, 37.445] },
      { name: "Orama", coords: [25.326158, 37.425753] },
      { name: "Cantera", coords: [25.326300, 37.445947] }
    ],
    nightClub: [
      { name: "Toy Room Mykonos", coords: [25.326497, 37.445694] },
      { name: "Semeli The Bar", coords: [25.326511, 37.445697] },
      { name: "We❤️Myk", coords: [25.329436, 37.447578] },
      { name: "Void", coords: [25.328611, 37.444306] },
      { name: "Bombonierre", coords: [25.328100, 37.444981] },
      { name: "Queen of Mykonos", coords: [25.328308, 37.445203] },
      { name: "Tabu", coords: [25.328306, 37.447200] },
      { name: "Cavo Paradiso", coords: [25.360436, 37.408081] },
      { name: "Tape", coords: [25.329428, 37.447842] }
    ]
  };
